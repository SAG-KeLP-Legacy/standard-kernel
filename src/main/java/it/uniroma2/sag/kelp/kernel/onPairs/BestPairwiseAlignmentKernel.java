 ...
 * 
 * \(K( < x_1, x_2 >, < y_1,y_2 > ) = softmax(BK(x_1, y_1) \cdot BK(x_2, y_2) , BK(x_1, y_2) \cdot BK(x_2, y_1))\) <br>
 * 
 * <p>
 * where \(softmax(a,b) = \frac{log(e^{Fa} + e^{Fb})}{F}\) with F being a large number (F=100 is enough)
 * 
 * where BK is another kernel the kernel on pairs relies on. <br> 
 *
 * The pairwise product kernel is described in: 
...
public class BestPairwiseAlignmentKernel extends KernelOnPairs{
	 ...
	 */
	public BestPairwiseAlignmentKernel(Kernel baseKernel, boolean intraPairSimProduct){
	...
	public float kernelComputationOverPairs(Example exA1, Example exA2, Example exB1,
			Example exB2) {
		float kernelValue = Math.softmax(this.baseKernel.innerProduct(exA1, exB1) * this.baseKernel.innerProduct(exA2, exB2) 
